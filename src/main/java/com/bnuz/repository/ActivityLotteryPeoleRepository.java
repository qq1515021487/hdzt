...
//    public List<ActivityLotteryPeopleEntity> findByPridLeftJoin
    ...
    @Query(value = "SELECT c.* FROM t_activity_attendent_info as a left join t_activity_attendent as b on a.attendent_id = b.attendent_id " +
            "left join t_activity_lottery_people as c on b.prid = c.prid WHERE a.attendent_id = :attendentId ORDER BY c.gain_time DESC", nativeQuery = true)
    ...
    List<ActivityLotteryPeopleDto> findActivityLotteryPeopleDtosByPridOrderByGainTimeDesc(String prid);
}